...
const Reaction = () => {
    ...
    const resetState = () => {
        ...
        setName('');
        setScoreSaved(false);
        setSaveScoreDisabled(false);
    };

    // useEffect hook to reset state and fetch the top scores whenever the location changes
    useEffect(() => {
        resetState(); // Reset state whenever the location changes
        defineUnits();
        defineDp();
        defineGameName();
        getTopScores(); // Automatically fetch top scores when the page loads
    }, [location]); // Dependency on location ensures reset on location change
